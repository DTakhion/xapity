...
#     if currency == "CLP":
#         formatted = f"${amount:,.0f}".replace(",", ".") + " CLP"
#     elif currency == "UF":
#         formatted = f"{str(amount).replace('.', ',')} UF"
#     else:
...
# def _get_days_text(benefit: dict[str, Any]) -> str | None:
#     for rule in benefit.get("rules", []):
#         days = rule.get("days")
#         day_type = rule.get("dayType")

#         if days is None:
#             continue

#         if day_type:
#             return f"{days} días {day_type}"
#         return f"{days} días"
...
def build_deterministic_answer(
    ...
    if summary:
        ...
        answer_parts.append(
            f"Según la información estructurada disponible, el beneficio corresponde a {title}."
        )
    ...
    days_text = _get_days_text(benefit)
    if days_text:
        answer_parts.append(f"El beneficio contempla {days_text}.")
...
if __name__ == "__main__":
    ...
    for question in test_questions:
        ...
        print("\n========================")
        print(f"Pregunta: {question}")
        print(f"Matched: {match.get('matched')}")
        print(f"Confidence: {match['confidence']}")
