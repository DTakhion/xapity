...
def main() -> None:
    ...
    if not login_url or not username or not password:
        raise RuntimeError(
            "Faltan variables en .env: LUCA_LOGIN_URL, LUCA_USERNAME, LUCA_PASSWORD"
        )
    ...
    with sync_playwright() as p:
        ...
        # Selectores flexibles. Si fallan, los ajustamos mirando el HTML real.
        ...
        if captured_auth_headers:
            token = captured_auth_headers[-1].replace("Bearer ", "").strip()
        ...
        if token:
            ...
            print("No se encontró token automáticamente")
            print("Puede estar en cookies, IndexedDB o venir en una respuesta específica.")
